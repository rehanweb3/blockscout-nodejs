  ...
  const onFormSubmit: SubmitHandler<FormFields> = React.useCallback(async(data) => {
    ...
    try {
      ...
      toaster.success({
        ...
        description: 'Your contract source code has been received. Verification may take a moment.',
      });

      await delay(300);
      reset(getDefaultValues(methodFromQuery, config, hash, []));
      return;
    } catch (error) {
      toaster.error({
        title: 'Submission failed',
        description: 'Could not submit contract for verification. Please try again.',
      });
      return;
    }
  }, [ apiFetch, config, hash, methodFromQuery, reset, setError, trackContract ]);
