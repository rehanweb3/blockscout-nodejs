...
const ContractDetails = ({ addressData, channel, mainContractQuery }: Props) => {
  ...
  const [ selectedItem, setSelectedItem ] = React.useState<AddressImplementation | undefined>(
    () => sourceItems.find((item) => item.address_hash === sourceAddress) || sourceItems[0],
  );

  React.useEffect(() => {
    if (!mainContractQuery.isPlaceholderData) {
      setSelectedItem(prev => prev ?? (sourceItems.find((item) => item.address_hash === sourceAddress) || sourceItems[0]));
    }
